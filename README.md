...
- Отслеживает новые ноды, низкий заряд батареи, прямые соседи
...
1. Чат 1 (публичный) — для канала 0
   - Создайте группу → запомните Chat ID
2. Чат 2 (приватный) — для канала 1
   - Создайте группу → запомните Chat ID
...
3. Добавьте бота в публичный и приватный групповые чаты
...
Для отправки сообщения в директ другой ноде напишите @КОРОТКОЕ_ИМЯ тест
...
| Обновление Базы имен происходит раз в пол часа, либо вручную через команду в /help |
